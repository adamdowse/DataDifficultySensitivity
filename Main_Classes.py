...
def Main(config):
    ...
    for c in range(data_obj.num_classes):
        ...
        wandb.log({'FIM_'+str(c):classFIM},step=0)
    

    

    #Training
    epoch_num = 1 #this is the epoch number
    adjusted_epoch_num = 1 #this is the epoch number acounting for percentage epochs
    while epoch_num <= config.epochs and not model.early_stop(adjusted_epoch_num):
        ...
        for c in range(data_obj.num_classes):
            ...
            ds,num_batches = data_obj.init_data(FIM_BS,train=True,distributed=True,shuffle=True)
            classFIM = model.calc_dist_FIM(ds,num_batches,FIM_BS)
            wandb.log({'FIM_'+str(c):classFIM},step=epoch_num)
